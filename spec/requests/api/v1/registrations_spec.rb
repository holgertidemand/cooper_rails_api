RSpec.describe 'User Registration', type: :request do
  ...
  context 'returns an error message when user submits' do
    ...
    it 'an invalid email address' do
      ...
      expect(response_json['errors']['email'][1]).to eq 'is not an email'
      expect(response.status).to eq 422
    end

    it 'an already registered email' do
      factory_bot_create = FactoryBot.create(:user, email: 'example@craftacademy.se',
                                              password: 'password',
                                              password_confirmation: 'password')
      factory_bot_create
      ...
      expect(response_json['errors']['email'][0]).to eq 'has already been taken'
      expect(response.status).to eq 422
    end
  end
end